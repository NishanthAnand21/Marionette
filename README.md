...
**ATLAS has the techniques. Praxis runs them.**
...
Praxis closes the loop everyone rebuilds by hand:
...
```bash
praxis list                       # list techniques + ATLAS/OWASP mapping
praxis validate                   # lint the technique pack + fleet file
praxis run                        # run all techniques against the built-in range
praxis run --technique PRX-0003   # run one
praxis run --tactic persistence   # run one tactic
praxis run --json r.json --junit j.xml   # machine-readable output for CI

# the negative control: the same pack against a *defended* agent
praxis run --target hardened      # most techniques must FAIL here

# detection content over the event stream
praxis run --events e.jsonl
praxis rules run --events e.jsonl

# point at a real MCP server
praxis run --target mcp  --command "python3 my_server.py"
praxis run --target http --command "https://host/mcp"
praxis run --target callable --command "myapp.agents:build_target"

# run a whole fleet in parallel
praxis run --targets-file targets.yaml --workers 8
praxis run --targets-file targets.yaml --tag prod --fail-fast
praxis targets --list-kinds

# rug-pull detection (the postmark-mcp signature)
praxis snapshot --target mcp --command "python3 my_server.py" --out t0.json
# ... later, after an update ...
praxis snapshot --target mcp --command "python3 my_server.py" --out t1.json
praxis drift t0.json t1.json      # exits nonzero on high-severity drift
```
...
```
  techniques/*.yaml          declarative steps + assertions, one file per technique
        ...
        ▼
  praxis/targets/*           adapter: mock | hardened | mcp | http | callable
        │  every adapter normalizes what it observes into ...
        ▼
  praxis/schema.py           AgentEvent: type, principal, authority, provenance, data
        │
...
from your own agent, with Praxis nowhere in the picture. `run_id` and
`technique_id` are *banned* at rule-parse time so a rule cannot cheat by keying
on the fact that Praxis generated the traffic.
...
crash and not a false pass. `praxis validate` enforces that every verb a
...
2. **Target adapter layer** (`praxis/targets/`) — techniques are written once
   against an abstract `Target`. Five kinds ship; see below.
3. **Agent event schema** (`praxis/schema.py`) — normalized, OCSF-flavoured
...
4. **Rules engine** (`praxis/rules.py`, `rules/`) — 10 detection rules over
   that schema, evaluated offline against a JSONL event stream.
5. **Drift monitor** (`praxis/drift/`) — snapshot + hash tool descriptions,
...
```
$ praxis run                      # vulnerable range
total: 34 passed, 0 failed, 0 skipped, 0 errored  — 1 target(s) in 2ms

$ praxis run --target hardened    # same pack, defended agent
  [✓ PASS ] PRX-0005  Tool and Authority Enumeration
  [✗ FAIL ] PRX-0001  Indirect Prompt Injection via Tool Output
...
- **PRX-0005** (tool enumeration) — legitimately non-discriminating. A hardened
  agent still has tools and still advertises them; enumeration establishes
  attack surface and seeds the drift snapshot.

The list has only ever shrunk. PRX-0002 and PRX-0006 were genuine tautologies
caught by this test and rewritten to assert on the poisoned description and the
injected privileged read; PRX-0008 was weak by construction until the hardened
...
|----|-----------|-------|-------|--------|
| PRX-0001 | Indirect Prompt Injection via Tool Output | `AML.T0051.001` | ASI01, ASI02 | execution |
| PRX-0002 | Tool Description Poisoning (Rug Pull) | `AML.T0110.000, AML.T0109` | ASI04 | persistence |
| PRX-0003 | Confused Deputy via Delegated Authority | `AML.T0053, AML.T0051.001` | ASI03 | privilege-escalation |
| PRX-0004 | Agent Memory Poisoning | `AML.T0080.000` | ASI06 | persistence |
| PRX-0005 | Tool and Authority Enumeration | `AML.T0084.001` | ASI02 | discovery |
| PRX-0006 | Excessive Agency Data Exfiltration | `AML.T0086` | ASI02 | exfiltration |
| PRX-0007 | Injection-to-Memory Persistence Chain | `AML.T0051.001, AML.T0080.000` | ASI01, ASI06 | persistence |
| PRX-0008 | Silent Authority Escalation | `AML.T0053, AML.T0081` | ASI03 | privilege-escalation |
| PRX-0009 | Tool Poisoning via Implementation | `AML.T0110.001` | ASI04, ASI02 | persistence |
| PRX-0010 | Tool Poisoning via Runtime Response | `AML.T0110.002` | ASI01, ASI02 | persistence |
| PRX-0011 | RAG Corpus Poisoning | `AML.T0070` | ASI06, ASI01 | persistence |
| PRX-0012 | Thread Context Poisoning | `AML.T0080.001` | ASI06, ASI01 | persistence |
| PRX-0013 | Modify AI Agent Configuration | `AML.T0081` | ASI03, ASI04 | defense-evasion |
| PRX-0014 | LLM Prompt Self-Replication | `AML.T0061, AML.T0080.000` | ASI06, ASI08 | persistence |
| PRX-0015 | Credentials from Agent Configuration | `AML.T0083` | ASI03 | credential-access |
| PRX-0016 | Tool Credential Harvesting | `AML.T0098` | ASI02 | credential-access |
| PRX-0017 | Unsecured Credentials on Reachable Storage | `AML.T0055` | ASI02 | credential-access |
| PRX-0018 | Tool Definition Disclosure via Registry Probe | `AML.T0084.001` | ASI04 | discovery |
| PRX-0019 | Discover Tool Call Chains | `AML.T0084.003` | ASI02 | discovery |
| PRX-0020 | System Prompt Extraction | `AML.T0056` | ASI01 | exfiltration |
| PRX-0021 | Triggered Prompt Injection | `AML.T0051.002` | ASI01, ASI06 | execution |
| PRX-0022 | Delayed Execution of LLM Instructions | `AML.T0094` | ASI01, ASI06 | defense-evasion |
| PRX-0023 | Deploy Sub-Agent With Inherited Authority | `AML.T0103` | ASI03, ASI07 | execution |
| PRX-0024 | False RAG Entry Injection | `AML.T0071` | ASI06, ASI09 | defense-evasion |
| PRX-0025 | Agentic Resource Consumption Loop | `AML.T0034.002` | ASI02, ASI08 | impact |
| PRX-0026 | Data Destruction via Agent Tool Invocation | `AML.T0101` | ASI02, ASI01 | impact |
| PRX-0027 | Embedded Knowledge Source Enumeration | `AML.T0084.000` | ASI04 | discovery |
| PRX-0028 | Activation Trigger Probing | `AML.T0084.002` | ASI01 | discovery |
| PRX-0029 | Bulk Collection from a RAG Database | `AML.T0085.000` | ASI02 | collection |
| PRX-0030 | Connected-Tool Collection Sweep | `AML.T0085.001` | ASI02 | collection |
| PRX-0031 | Credential Harvesting from the Retrieval Corpus | `AML.T0082` | ASI03 | credential-access |
| PRX-0032 | Poisoned Record in a Connected Data Source | `AML.T0099` | ASI06, ASI01 | persistence |
| PRX-0033 | Poisoned Agent Tool from a Community Registry | `AML.T0011.002` | ASI04, ASI02 | execution |
| PRX-0034 | Prompt Infiltration via a Public Contact Form | `AML.T0093` | ASI01, ASI06 | initial-access |
...
Multi-step chaining (PRX-0007) is deliberate: atomic-only frameworks (Stratus,
...
fabricated or malformed id fails `praxis validate`.
...
exist in that file at all. Praxis pins the `dist/v6/` release instead.
...
shaped like `docs/event-schema.md` — Praxis need not have generated it.

```
$ praxis rules list
PRXR-0001  HIGH          Planner acted on an instruction from untrusted content
    stable  [AML.T0051.001,AML.T0053]
PRXR-0002  CRITICAL      Live tool description carries model-facing instructions
    stable  [AML.T0110.000,AML.T0109]
PRXR-0003  CRITICAL      Registered tool shadows a name the planner already trusts
    stable  [AML.T0110,AML.T0072]
PRXR-0004  MEDIUM        Tool registry mutated mid-session
    experimental  [AML.T0110]
PRXR-0005  CRITICAL      Privileged tool invoked on behalf of untrusted content
    stable  [AML.T0053,AML.T0055]
PRXR-0006  HIGH          Durable memory or retrieval corpus written from untrusted content
    stable  [AML.T0070,AML.T0071]
PRXR-0007  HIGH          Credential material returned into the model's context
    stable  [AML.T0055,AML.T0057]
PRXR-0008  CRITICAL      Outbound tool call driven by the agent's own standing instructions
    stable  [AML.T0056,AML.T0024.002]
PRXR-0009  HIGH          Elevated authority granted and exercised within one session
    experimental  [AML.T0054,AML.T0053]
PRXR-0010  MEDIUM        Self-sustaining tool loop -- call budget exceeded
    stable  [AML.T0034.002]

10 rules
```

`praxis rules validate` lints the pack, including a warning on any rule with no
`falsepositives:` — a rule without a false-positive story is how alert fatigue
starts:

```
$ praxis rules validate
all 10 rules valid
```

### The compose flow

`praxis run --events FILE` writes the raw event stream; `praxis rules run
...
```
$ praxis run --events e.jsonl --quiet
total: 34 passed, 0 failed, 0 skipped, 0 errored  — 1 target(s) in 2ms

$ praxis rules run --events e.jsonl
[CRITICAL] PRXR-0002  Live tool description carries model-facing instructions
    1 matching event(s)
[CRITICAL] PRXR-0003  Registered tool shadows a name the planner already trusts
    2 matching event(s)
[CRITICAL] PRXR-0005  Privileged tool invoked on behalf of untrusted content
    5 matching event(s)
[CRITICAL] PRXR-0008  Outbound tool call driven by the agent's own standing instructions
    1 matching event(s)
[HIGH] PRXR-0001  Planner acted on an instruction from untrusted content
    22 matching event(s)
[HIGH] PRXR-0006  Durable memory or retrieval corpus written from untrusted content
    6 matching event(s)
[HIGH] PRXR-0007  Credential material returned into the model's context
    6 matching event(s)
[HIGH] PRXR-0009  Elevated authority granted and exercised within one session
    22 matching event(s)
[MEDIUM] PRXR-0004  Tool registry mutated mid-session
    10 matching event(s)
[MEDIUM] PRXR-0010  Self-sustaining tool loop -- call budget exceeded
    4 matching event(s)
...
```
$ praxis rules run --events e.jsonl --coverage
  ✓ PRX-0022  Delayed Execution of LLM Instructions
      PRXR-0001,PRXR-0005,PRXR-0006
  ✓ PRX-0023  Deploy Sub-Agent With Inherited Authority
      PRXR-0001,PRXR-0005,PRXR-0007,PRXR-0009
  ...
```
...
```
$ praxis run --target hardened --events h.jsonl --quiet
total: 2 passed, 24 failed, 0 skipped, 0 errored  — 1 target(s) in 1ms

$ praxis rules run --events h.jsonl
[HIGH] PRXR-0007  Credential material returned into the model's context
    1 matching event(s)
[HIGH] PRXR-0009  Elevated authority granted and exercised within one session
    8 matching event(s)
...
emits machine-readable output. **`praxis rules run` exits 1 when any rule fires
and 0 when none do** — inverted relative to `praxis run`, because here a firing
...
Praxis is a **defensive** adversary-emulation tool: run it against your own
agents and MCP servers to validate that your detections fire. The bundled range
(`praxis/range/`) is intentionally vulnerable and is the safe default target.
...
full 26-technique pack (`praxis run --targets-file … --workers N`, wall clock
...
```
$ praxis run --target mcp --command "python3 -c 'import time; time.sleep(99)'" --timeout 2
→ mcp

mcp (mcp)
    [PRX-E102] mcp target 'mcp' timed out after 2.0s awaiting initialize
...
| code | meaning |
|------|---------|
| `PRX-E101/102/103/104/105` | target: connect / timeout / protocol / crash / unsupported capability |
| `PRX-E201/202/203` | technique: parse / validation / step |
| `PRX-E301/302` | config: targets file, unknown kind |

Exit codes are per-command, not per-error-class:

| command | 0 | 1 | 2 | 3 | 4 |
|---|---|---|---|---|---|
| `praxis run` | all pass | a detection failed | a target errored, or bad config | — | unknown technique id |
| `praxis validate` | valid | any lint error | — | — | — |
| `praxis snapshot` | captured | — | bad config / unknown kind | target unreachable | — |
| `praxis drift` | no drift | high-severity drift, or a snapshot file that will not load | — | — | — |
| `praxis rules run` | **no rule fired** | **a rule fired** | events file missing | — | — |

`praxis validate` collapses everything to 1 — it is a linter and reports every
...
Inside `praxis run` a target error is *contained* — it is reported per target
and folded into exit **2**, so an unreachable server never masks the rest of
the matrix. The bare `PRX-E1xx → 3` mapping applies to commands that drive a
single target and cannot continue without it, such as `praxis snapshot`.
...
Praxis is pointed at servers that may be actively malicious, so target-supplied
...
| Terminal repaint | A tool description carrying `ESC[2K CR` erased the line it printed on, making `praxis drift` display "no drift detected" while poisoning a tool | control chars and bidi overrides rendered as visible escapes |
...
catch. `praxis rules validate --against` checks empirically:

```console
$ praxis run --events corpus.jsonl
$ praxis rules validate --against corpus.jsonl
[ERROR] rule PRXR-9999 matched 0 of 198 events in corpus.jsonl — it may never
