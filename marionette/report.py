...
def _error_block(res: Any, indent: str, color: bool) -> list[str]:
    ...
    code = detail.get("code") or "MAR-E000"
...
def sanitize(text: Any, limit: int = MAX_UNTRUSTED) -> str:
    ...
    printed on and rewrite it -- letting a hostile server make `marionette drift`
    ...
    for run in matrix.runs:
        ...
        for res in run.results:
            ...
            case = ET.SubElement(suite, "testcase", {
                ...
                "classname": xml_safe(f"marionette.{run.target_name}"),
