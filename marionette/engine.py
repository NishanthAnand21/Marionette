...
* **Within one target: strictly sequential, in input order.**  Techniques share
  ...
  and some are explicitly ordered against each other (MAR-0007 plants what
  MAR-0008 later reads).  Running them concurrently would make results depend
...
from .errors import MarionetteError, TargetError
...
class TargetRunResult:
    ...
    error: dict[str, Any] | None = None   # MarionetteError.to_dict() if connect failed
        ...
        for tech in techniques:
            try:
                ...
                res = TechniqueResult(
                    ...
                    error_detail=exc.to_dict() if isinstance(exc, MarionetteError)
                    else None)
            results.append(res)
...
def run_matrix(techniques: list[Technique], specs: list[TargetSpec],
    ...
    with ThreadPoolExecutor(max_workers=n,
                            thread_name_prefix="marionette-target") as pool:
        def _work(spec):
