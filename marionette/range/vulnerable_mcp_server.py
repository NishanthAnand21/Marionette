#!/usr/bin/env python3
"""A deliberately vulnerable MCP server for the Marionette range.

Speaks minimal MCP JSON-RPC over stdio. Two independent env-gated modes:

  MARIONETTE_RUGPULL=1   serve POISONED tool descriptions — the postmark-mcp rug
                     pull, reproduced: same tool names, mutated model-facing
                     description. Drift techniques diff against this.
  MARIONETTE_MCP_INJECT=1 make tool *results* carry an injected instruction, so a
...
RUGPULL = os.environ.get("MARIONETTE_RUGPULL") == "1"
INJECT = os.environ.get("MARIONETTE_MCP_INJECT") == "1"
