...
from marionette.technique import Technique, load_dir
from marionette.runner import run_technique
from marionette.drift import Snapshot, diff
from marionette.targets.base import ToolSpec
from marionette.schema import AgentEvent, TOOL_CALL
...
def test_confused_deputy_actually_reaches_secret():
    target = build("mock")
    tech = next(t for t in all_techniques() if t.id == "MAR-0003")
    with target:
...
def test_drift_severity_ordering_description_change_is_worst():
    from marionette.schema import SEVERITIES
...
def test_event_jsonl_round_trip_preserves_fields(tmp_path):
    from marionette.schema import MEMORY_WRITE, read_jsonl, write_jsonl
    ...
    events = [
        AgentEvent(type=TOOL_CALL, actor="agent-1", target="mock",
                   ...
                   run_id="run-abc", technique_id="MAR-0001",
...
def test_read_jsonl_skips_blank_lines(tmp_path):
    ...
    path.write_text(AgentEvent(type=TOOL_CALL).to_json() + "\n\n\n",
                    encoding="utf-8")
    from marionette.schema import read_jsonl
...
def test_assertion_field_contains_on_nested_data_path():
    from marionette.detection import Assertion
    from marionette.schema import MEMORY_WRITE
