...
`praxis validate` is the authoring feedback loop.  It works on *raw* YAML
rather than on constructed :class:`~praxis.technique.Technique` objects on
...
def validate_file(path: str) -> list[Problem]:
    ...
    if not _blank(raw_id):
        if not isinstance(raw_id, str) or not ID_RE.match(raw_id):
            out.append(Problem(
                path,
                f"`id` {raw_id!r} does not match the required format PRX-####",
                ERROR, tid))
