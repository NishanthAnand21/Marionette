"""praxis — command-line entry point.
    ...
    praxis run                           run everything against the mock range
    praxis run --targets-file f.yaml     run the matrix across a fleet
    praxis validate                      lint technique YAML and the fleet file
    praxis targets --list-kinds          show registered adapter kinds
    praxis snapshot --out snap.json      fingerprint a target's tools
    praxis drift OLD.json NEW.json       diff two snapshots (rug-pull detector)
    praxis run --events e.jsonl          also save the raw event stream
    praxis rules list                    list the shipped detection rules
    praxis rules validate                lint the rule pack
    praxis rules run --events e.jsonl    evaluate rules over a saved stream
...
from .errors import (ConfigError, PraxisError, TargetError, TechniqueError,
...
def _exit_code_for(err: PraxisError) -> int:
...
def _write_events(path: str, matrix, args, color: bool) -> None:
    ...
    This is the other half of `praxis rules run`: the range produces events,
...
def cmd_rules_validate(args) -> int:
    ...
    n_err = sum(1 for p in problems if "ERROR" in p or "PRX-E" in p)
    for line in problems:
        print(line)
    if not problems:
...
def cmd_rules(args) -> int:
    # `praxis rules` with no subcommand: show what is available rather than
    # an argparse stack trace.
    raise ConfigError("`praxis rules` needs a subcommand",
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="praxis",
        description="ATLAS has the techniques. Praxis runs them.")
    ...
    rr.add_argument("--events", required=True, metavar="PATH",
                    help="JSONL event stream from `praxis run --events`")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if getattr(args, "debug", False):
            raise
        print(f"[PRX-E000] unexpected error: {exc}\n"
