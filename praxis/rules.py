"""Portable detection rules over the Praxis agent event schema.
...
here is plain YAML over :mod:`praxis.schema` events, so the same file can be
...
Grammar (one rule = one YAML document)::
    ...
    covers: [PRX-0001]           # documentation only; coverage is measured
    atlas: [AML.T0051.001]
    falsepositives: [...]
...
# Keys that would make a rule self-fulfilling: they exist only because Praxis
            ...
            for key, value in block.items():
                ...
                if fieldname.split(".")[0] in FORBIDDEN_FIELDS:
                    raise ConfigError(
                        ...
                        hint="rules must detect behaviour, not Praxis "
...
def load_events(path: str) -> list[AgentEvent]:
    try:
        ...
        raise ConfigError(f"could not read events from {path}: {exc}",
                          hint="produce one with `praxis run --events FILE`") from exc
    except (ValueError, TypeError) as exc:
        raise ConfigError(f"{path} is not a valid Praxis event JSONL: {exc}",
                          hint="one JSON object per line, as written by "
                               "`praxis run --events`") from exc
...
def coverage(rules: Iterable[Rule], events: Iterable[AgentEvent]) -> CoverageReport:
    ...
    for ev in events:
        ...
        if ev.type == "praxis.technique.start":
