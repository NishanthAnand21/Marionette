...
Praxis target) advertises, hash each tool's model-facing description, and diff
    ...
    def load(cls, path: str) -> "Snapshot":
        ...
        try:
            ...
            raise ConfigParseError(f"snapshot not found: {path}",
                                   hint="run `praxis snapshot --out <path>` first",
        ...
        try:
            ...
            raise ConfigParseError(
                ...
                hint="re-capture it with `praxis snapshot`",
                context={"path": path}) from None
