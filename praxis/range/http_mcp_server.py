...
drift apart and quietly invalidate a cross-transport comparison.
  ...
  PRAXIS_RUGPULL=1     serve POISONED tool descriptions (the postmark-mcp rug pull)
  PRAXIS_MCP_INJECT=1  make tool *results* carry an injected instruction
  PRAXIS_HTTP_SSE=1    answer with text/event-stream instead of application/json,
...
:func:`serve` to drive it in-process from a test.
    ...
    python3 -m praxis.range.http_mcp_server          # ephemeral port
    python3 -m praxis.range.http_mcp_server 8931     # fixed port
"""
...
def _flags() -> tuple[bool, bool, bool]:
    ...
    return (os.environ.get("PRAXIS_RUGPULL") == "1",
            os.environ.get("PRAXIS_MCP_INJECT") == "1",
            os.environ.get("PRAXIS_HTTP_SSE") == "1")
...
def serve(port: int = 0, host: str = "127.0.0.1"):
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="praxis-http-range",
                              daemon=True)
    thread.start()
    return httpd, thread
