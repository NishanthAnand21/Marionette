...
Praxis must be able to point at a LangGraph graph, a CrewAI crew, or someone's
...
class CallableTarget(Target):
    """Wrap a user-supplied agent shim as a Praxis target.
    ...
    ``praxis targets --list-kinds`` and the validator can see what this kind
    could ever offer.
    """
    ...
    def connect(self) -> None:
        ...
        if not self._is_shim(self.impl):
            raise TargetConnectError(
                ...
                hint="see praxis/targets/callable.py for the expected protocol",
    ...
    def call_tool(self, tool: str, args: dict[str, Any]) -> ToolResult:
        ...
        self.emit_tool_call(tool, args, principal="operator", provenance="praxis")
        try:
            raw = fn(tool, args)
        except Exception as exc:  # noqa: BLE001
